...
export default function MySubmissions() {
  ...
  const fetchSubmissions = async (userId: string | null) => {
    try {
      setIsLoading(true)
      const response = await fetch(`${API_URL}/api/startupSubmission/getSubmissionsByUserId/${userId}`)
      const data = await response.json()
      setSubmissions(data)
  ...
  const handleViewPDF = async (submissionId: string) => {
    try {
      const response = await fetch(`${API_URL}/api/startupSubmission/getStartupSubmissionById/${submissionId}?includeFile=true`)
      const data = await response.json()
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`${API_URL}/api/startupSubmission/deleteStartupSubmission/${selectedSubmission._id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
            ...
            <>
              ...
              <div className="space-y-4">
                <div className="flex gap-2 mb-4">
                  ...
                  <button
                    onClick={() =>
                      handleDownloadPDF(selectedSubmission._id, selectedSubmission.pitchDeckFile.name)
                    }
                    className="px-3 py-1.5 bg-secondary text-secondary-foreground rounded-lg hover:bg-secondary/90 transition-all text-sm"
                  >
                    Download PDF
